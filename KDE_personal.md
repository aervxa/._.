...
If you're not on a clean installtion, make sure to backup any file before overwriting. However, if you are on a clean install, don't fret.
...
### I won't list the whole installations of each thing i used, but what i will do is list it, and what it is.
...
> feel free to overwrite the config files

> You could either copy the files manually, or clone the repo and: `cp <repo>/<path> <location>`

- `spectacle` (path: [`.config/spectaclerc`](.config/spectaclerc)) (location: `~/.config/spectaclerc`)
